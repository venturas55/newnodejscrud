...
router.get("/plantilla/:columnaA", async (req, res) => {
  ...
  var fotitos = helpers.listadoFotos(columnaA);
  console.log("==>" + fotitos);
...
//UPDATE
router.get("/edit/:columnaA", helpers.isAuthenticated, async (req, res) => {
  const { columnaA } = req.params;
  const item = await db.query("SELECT * FROM items1 WHERE columnaA=?", [columnaA,]);
  //console.log(item[0]);
  res.render("items1/edit", { item: item[0] });
});
router.post("/edit/:columnaA", helpers.isAuthenticated, async (req, res) => {
...
//DELETE
router.get("/delete/:columnaA", helpers.isAuthenticated, async (req, res) => {
...
router.get("/fotos/:columnaA", async (req, res) => {
  ...
  var fotos = helpers.listadoFotos(columnaA);
